...
public class ProductController {
    ...
    public ResponseEntity<String> updateProduct(@PathVariable int id, @RequestPart Product product, @RequestPart MultipartFile imagFile){
        Product product1 = service.updateProduct(id, product, imageFile);
        if (product1 != null)
            return new ResponseEntity<>("updated product", HttpStatus.OK);
        else
            return new ResponseEntity<>("Failed to update", HttpStatus.B)
    }
}
